...
router.post('/save-course', async (req, res) => {
    try {
        ...
        // Helper to normalize difficulty strings (easy -> Easy, beginner -> Beginner)
        const normalizeDifficulty = (diff, fallback = "Beginner") => {
            if (!diff) return fallback;
            const str = diff.toLowerCase();
            if (str.includes('easy') || str.includes('begin')) return "Beginner";
            if (str.includes('medium') || str.includes('inter')) return "Intermediate";
            if (str.includes('hard') || str.includes('adv')) return "Advanced";

            // Fallback for direct matches not caught by includes
            return diff.charAt(0).toUpperCase() + diff.slice(1).toLowerCase();
        };

        const normalizeQuestionDifficulty = (diff) => {
            if (!diff) return "Easy";
            const str = diff.toLowerCase();
            if (str.includes('easy')) return "Easy";
            if (str.includes('medium')) return "Medium";
            if (str.includes('hard')) return "Hard";
            return "Medium"; // Safe fallback for questions
        };

        // Generate a unique course ID
        const courseId = `ai-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;

        // Map and normalize lessons
        const normalizedLessons = roadmap.lessons.map((lesson, index) => {
            // Further normalize stages if they exist
            if (lesson.stages && lesson.stages.practice && lesson.stages.practice.questions) {
                lesson.stages.practice.questions = lesson.stages.practice.questions.map(q => ({
                    ...q,
            ...
            return {
                ...
                xp: lesson.xp || 100,
                type: "lesson",
                difficulty: normalizeDifficulty(roadmap.difficulty),
...
function generateFallbackRoadmap(goal, experience, timeCommitment, interests) {
    ...
    const result = {
        ...
        difficulty: experience.charAt(0).toUpperCase() + experience.slice(1).toLowerCase(),
        lessons: phases.map((phase, idx) => ({
            id: `lesson-${idx + 1}`,
            title: phase.title,
            xp: 100,
            stages: {
                read: {
                    xp: 20,
                    introduction: `Welcome to ${phase.title}. This phase focuses on ${phase.topics.join(', ')}.`,
                    sections: phase.topics.map(topic => ({
                        title: topic,
                        content: `In this section, we will explore the core concepts of ${topic} and how it relates to your goal of ${goal}.`
                    }))
                },
                practice: {
                    xp: 50,
                    questions: [
                        {
                            ...
                            difficulty: experience.charAt(0).toUpperCase() + experience.slice(1).toLowerCase(),
                            xpReward: 25,
                            question: `Which of these is a key topic in ${phase.title}?`,
                            options: [phase.topics[0], "Something unrelated", "A different domain", "None of the above"],
                            correctAnswer: 0,
                            explanation: `${phase.topics[0]} is one of the main areas we covered.`
                        }
                    ]
                },
                notes: {
                    xp: 30,
                    summary: `You've completed the foundation for ${phase.title}.`,
                    keyTakeaways: phase.topics
                }
            }
        })),
        generatedFor: {
